...
mus=c(0.4,0.1)
generations=c(4,5)#,7,8,9,10)#,11,12)
...
for(ca in 1:length(integrases)){
    ...
    save(cascadevar,file=paste("intVar_int_",toString(nIntegrases),"_.rda",sep=""))
    save(cascadevar.tree,file=paste("intVar_int_",toString(nIntegrases),"_trees_.rda",sep=""))
}
